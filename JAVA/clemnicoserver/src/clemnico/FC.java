...
//				if (directionCollision.x!=0 ||directionCollision.y>0) {entity.setVx(0);}
	...
	public ArrayList<Player> concatenatePlayer(ArrayList<PlayerLocal> listPlayerLocal, ArrayList<PlayerWeb> listPlayerWeb){
		ArrayList<Player> listPlayer = new ArrayList<>();
		for (PlayerLocal playerLocal : listPlayerLocal) {
			listPlayer.add(playerLocal);
		}
		for (PlayerWeb playerWeb : listPlayerWeb) {
			listPlayer.add(playerWeb);
		}
		return listPlayer;
	}
